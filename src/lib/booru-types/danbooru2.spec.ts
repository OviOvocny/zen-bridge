...
test('gets an artist', t => {
  return b.artist(152501).then(content => {
    t.truthy(typeof content === 'object')
...
test('gets a comment', t => {
  return b.comment(1832633).then(content => {
    t.truthy(typeof content === 'object')
    t.truthy(content.creator)
...
test('adds comments to a Post', t => {
  ...
  return b.post(1).then(post => {
    t.truthy(post)
    return b.comments(post).then(populated => {
...
test('gets a note', t => {
  return b.note(8940).then(content => {
    t.truthy(typeof content === 'object')
    t.truthy(content.creator)
...
test('adds notes to a Post', t => {
  t.plan(2)
  return b.post(7).then(post => {
    t.truthy(post)
    return b.notes(post).then(populated => {
...
test('gets a pool', t => {
  return b.pool(5674).then(content => {
    t.truthy(typeof content === 'object')
    t.truthy(content.creator)
...
test('gets a wiki page', t => {
  return b.wiki(43568).then(content => {
    t.truthy(typeof content === 'object')
